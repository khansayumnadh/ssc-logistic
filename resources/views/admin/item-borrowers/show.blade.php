@extends('layouts.stisla.index', ['title' => 'Detail Peminjaman ' . $item_user->user->name, 'section_header' => 'Detail Peminjaman ' . $item_user->user->name])
...
<div class="row">
      ...
      <form action="" method="POST" class="item_borrower_form">
        ...
        <table class="table">
          ...
          <tr>
            <td>Dari Tanggal</td>
            <td>:</td>
            <td class="text-wrap">{{ date_format(date_create($item_user->date_start), 'd-m-Y') }}</td>
          </tr>
          <tr>
            <td>Sampai Tanggal</td>
            <td>:</td>
            <td class="text-wrap">{{ date_format(date_create($item_user->date_end), 'd-m-Y') }}</td>
          </tr>
          <tr>
            <td>Keterangan</td>
            <td>:</td>
  ...
  <div class="col-lg-5">
    ...
    <div class="py-4">
      <a href="{{ route('admin.item-borrowers.index') }}" class="btn btn-primary">Kembali</a>
      <button type="submit" class="btn btn-success" data-id="{{ $item_user->id }}" id="item-approved-button">Setujui</button>
      <button type="submit" class="btn btn-danger" data-id="{{ $item_user->id }}" id="item-not-approved-button">Tidak menyetujui</button>
    </div>
...
@push('modal')
@include('admin.item-borrowers.modal.user.show');
@include('admin.item-borrowers.modal.item.show')
@endpush
    ...
    $("#user-swal-show-button").click(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#organization_detail_show").html(organization);
        },
        error: function(data) {
          console.log(0);
          console.log(data);
